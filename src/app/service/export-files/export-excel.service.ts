...
export class ExportExcelService {
  ...
  public exportAsXLSXFile(json: any[], excelFileName: string, nomeInvertido: boolean): void {
    ...
    if (!nomeInvertido)
      ...
      this.saveAsExcelFileInvertido(excelBuffer, excelFileName, XLSX_TYPE, XLSX_EXTENSION);
  }

  public exportAsCSVFile(json: any[], excelFileName: string, nomeInvertido: boolean): void {
    const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(json);
    ...
    if (!nomeInvertido)
      ...
      this.saveAsExcelFileInvertido(csvOutput, excelFileName, XLSX_TYPE, CSV_EXTENSION);
  }
  ...
  private saveAsExcelFileInvertido(buffer: any, fileName: string, type: string, extension: string): void {
    const data: Blob = new Blob([buffer], { type: type });
    let dataLocal = DataUtils.dataLocalParaArquivo(new Date());
    FileSaver.saveAs(data, `${dataLocal}_${fileName}${extension}`);
  }
}
